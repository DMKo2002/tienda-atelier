...
export async function POST(req: NextRequest) {
  try {
    ...
    const [{ data: tenant }, { data: emailConfig }] = await Promise.all([
      ...
      supabase.from('store_config').select('email_from_name, reply_to').eq('tenant_id', tenantId).single(),
    ])
    ...
    const emailResult = await sendEmail({
      ...
      fromName: (emailConfig as any)?.email_from_name ?? storeName,
      ...((emailConfig as any)?.reply_to ? { replyTo: (emailConfig as any).reply_to } : {}),
